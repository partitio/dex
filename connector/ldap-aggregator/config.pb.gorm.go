...
type LdapConfigORM struct {
	...
	ClientCert         string
	ClientKey          string
	GroupSearch        *GroupSearchORM `gorm:"embedded;embedded_prefix:group_search_;foreignkey:LdapConfigId;association_foreignkey:Id"`
	Host               string
	Id                 string
	InsecureNoSSL      bool
	...
	RootCAData         []byte
	StartTLS           bool
	UserSearch         *UserSearchORM `gorm:"embedded;embedded_prefix:user_search_;foreignkey:LdapConfigId;association_foreignkey:Id"`
...
func (m *LdapConfig) ToORM(ctx context.Context) (LdapConfigORM, error) {
	...
	if m.GroupSearch != nil {
		tempGroupSearch, err := m.GroupSearch.ToORM(ctx)
		if err != nil {
			return to, err
		}
...
func DefaultStrictUpdateLdapConfig(ctx context.Context, in *LdapConfig, db *gorm1.DB) (*LdapConfig, error) {
	...
	if hook, ok := interface{}(&ormObj).(LdapConfigORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	filterGroupSearch := GroupSearchORM{}
	if ormObj.Id == "" {
		return nil, errors1.EmptyIdError
	}
	filterGroupSearch.LdapConfigId = new(string)
	*filterGroupSearch.LdapConfigId = ormObj.Id
	if err = db.Where(filterGroupSearch).Delete(GroupSearchORM{}).Error; err != nil {
		return nil, err
	}
	filterUserSearch := UserSearchORM{}
	if ormObj.Id == "" {
		return nil, errors1.EmptyIdError
	}
	filterUserSearch.LdapConfigId = new(string)
	*filterUserSearch.LdapConfigId = ormObj.Id
	if err = db.Where(filterUserSearch).Delete(UserSearchORM{}).Error; err != nil {
		return nil, err
	}
...
// DefaultPatchSetLdapConfig executes a bulk gorm update call with patch behavior
func DefaultPatchSetLdapConfig(ctx context.Context, objects []*LdapConfig, updateMasks []*field_mask1.FieldMask, db *gorm1.DB) ([]*LdapConfig, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors1.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*LdapConfig, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchLdapConfig(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
